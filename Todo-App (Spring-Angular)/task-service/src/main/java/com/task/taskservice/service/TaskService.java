...
public class TaskService {
    ...
    public Page<TaskResponse> list(Status status, String title, Pageable pageable) {
        boolean hasTitle = title != null && !title.isBlank();

        if (status != null && hasTitle) {
            return taskRepository.findAllByStatusAndTitleContainingIgnoreCase(status, title, pageable)
                    .map(this::toResponse);
        }
        ...
        if (hasTitle) {
            return taskRepository.findAllByTitleContainingIgnoreCase(title, pageable).map(this::toResponse);
        }
